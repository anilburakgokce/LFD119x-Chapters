...
void GPIO_Initialization(void)
{
  ...
  M_PSP_WRITE_REGISTER_32(GPIO2_INOUT, 0);             /* GPIO_INOUT */

  /* Configure Pushbuttons interrupts */
  M_PSP_WRITE_REGISTER_32(RGPIO2_INTE, 0x2);           /* RGPIO_INTE */
  M_PSP_WRITE_REGISTER_32(RGPIO2_PTRIG, 0x0);          /* RGPIO_PTRIG */
...
void PTC_Initialization(void){
  ...
  M_PSP_WRITE_REGISTER_32(RPTC_LRC,count_period);
  M_PSP_WRITE_REGISTER_32(RPTC_CTRL,0x21);
}

void PTC_ISR(void){
  // write LEDs
  M_PSP_WRITE_REGISTER_32(GPIO_LEDs, LEDs_count++);
...
void GPIO_ISR(void)
{
  /* check if the switch generated an interrupt */
  if(M_PSP_READ_REGISTER_32(RGPIO_INTS) & 0x10000){
    /* Reset the LEDs to 0 and restart the timer */
    LEDs_count = 0;
    PTC_ISR();
  ...
  /* check if the center pushbutton generated an interrupt */
  if(M_PSP_READ_REGISTER_32(RGPIO2_INTS) & 0x2){
    // alternate the speed
    count_period ^= slowStepPeriod ^ fastStepPeriod;

    // reinitialize the counter
    PTC_Initialization();

    /* Clear Pushbuttons interrupt */
...
int main(void)
{
  ...
  ExternalIntLine_Initialization(3, 8, PTC_ISR);      /* Initialize line IRQ3 with a priority of 6. Set PTC_ISR as the Interrupt Service Routine */
